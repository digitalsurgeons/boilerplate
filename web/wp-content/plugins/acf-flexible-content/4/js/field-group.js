...
;(function($) {
  acf_fc = {
    ...
    init: function() {
      ...
      this.$el.on('change', '.acf-fc-meta-display select', function(e) {
        ...
        // Set class
        $repeater
          .removeClass('layout-row')
          .removeClass('layout-table')
          .addClass('layout-' + $(this).val())
      })
    },
    ...
    remove: function($tr) {
      ...
      // create blank space
      $blank = $(
        '<tr style="height:' + $tr.height() + 'px"><td colspan="2"></td></tr>'
      )
    ...
    wipe_layout: function($el) {
      ...
      $el.find('[name]').each(function() {
        var name = $(this)
          .attr('name')
          .replace('[layouts][' + old_id + ']', '[layouts][' + new_id + ']')
  ...
  $(document).live('acf/field_form-open', function(e, field) {
    $(field)
      .find('.acf-fc-meta-display select')
      .each(function() {
        $(this).trigger('change')
      })
  })
