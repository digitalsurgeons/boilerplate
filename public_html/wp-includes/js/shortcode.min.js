  ...
  (function() {
    ;(wp.shortcode = {
      next: function(a, b, c) {
        var d,
          e,
          f = wp.shortcode.regexp(a)
        if (((f.lastIndex = c || 0), (d = f.exec(b))))
          return '[' === d[1] && ']' === d[7]
            ? wp.shortcode.next(a, b, f.lastIndex)
            : ((e = {
                index: d.index,
                content: d[0],
                shortcode: wp.shortcode.fromMatch(d)
              }),
              d[1] && ((e.content = e.content.slice(1)), e.index++),
              d[7] && (e.content = e.content.slice(0, -1)),
              e)
      },
      replace: function(a, b, c) {
        return b.replace(wp.shortcode.regexp(a), function(
          a,
          b,
          d,
          e,
      ...
      attrs: _.memoize(function(a) {
        var b,
          ...
          e = []
        for (
          b = /([\w-]+)\s*=\s*"([^"]*)"(?:\s|$)|([\w-]+)\s*=\s*'([^']*)'(?:\s|$)|([\w-]+)\s*=\s*([^\s'"]+)(?:\s|$)|"([^"]*)"(?:\s|$)|'([^']*)'(?:\s|$)|(\S+)(?:\s|$)/g,
            a = a.replace(/[\u00a0\u200b]/g, ' ');
          (c = b.exec(a));
          ...
          c[1]
            ...
            : c[3]
              ? (d[c[3].toLowerCase()] = c[4])
              : c[5]
                ? (d[c[5].toLowerCase()] = c[6])
                : c[7]
                  ? e.push(c[7])
                  : c[8]
                    ? e.push(c[8])
      ...
      fromMatch: function(a) {
        var b
        return (
          (b = a[4] ? 'self-closing' : a[6] ? 'closed' : 'single'),
          new wp.shortcode({ tag: a[2], attrs: a[3], type: b, content: a[5] })
        )
      }
    }),
      (wp.shortcode = _.extend(function(a) {
        ...
        ;(this.attrs = { named: {}, numeric: [] }),
          b &&
            (_.isString(b)
              ? (this.attrs = wp.shortcode.attrs(b))
              : _.isEqual(_.keys(b), ['named', 'numeric'])
                ? (this.attrs = b)
                : _.each(
                    a.attrs,
                    function(a, b) {
      ...
      _.extend(wp.shortcode.prototype, {
        ...
        string: function() {
          var a = '[' + this.tag
          return (
            _.each(this.attrs.numeric, function(b) {
              a += /\s/.test(b) ? ' "' + b + '"' : ' ' + b
            }),
