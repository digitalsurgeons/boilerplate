  ...
  var media = (function() {
    ...
    var htmlToDataSax = function(prefixes, html) {
      ...
      global$3({
        ...
        start: function(name, attrs) {
          ...
          if (name === 'script') {
            var videoScript = $_9fg1sigejfuw8poq.getVideoScriptMatch(
              prefixes,
              attrs.map.src
            )
            if (!videoScript) {
    ...
    var dataToHtml = function(editor, dataIn) {
      ...
      if (!data.source1) {
        global$2.extend(
          data,
          $_g969ozgbjfuw8pom.htmlToData(
            $_bmkzfegajfuw8pol.getScripts(editor),
            data.embed
          )
        )
        if (!data.source1) {
      ...
      if (data.embed) {
        ...
        var audioTemplateCallback = $_bmkzfegajfuw8pol.getAudioTemplateCallback(
          editor
        )
        var videoTemplateCallback = $_bmkzfegajfuw8pol.getVideoTemplateCallback(
          editor
        )
    ...
    var showDialog = function(editor) {
      ...
      var reserialise = function(update) {
        ...
        win
          .find('#embed')
          .value($_9oq1mlgkjfuw8pp0.updateHtml(data.embed, data))
      }
    ...
    var retainAttributesAndInnerHtml = function(
      ...
      if (innerHtml) {
        targetNode.attr(
          'data-mce-html',
          escape($_77d5ybgtjfuw8ppp.sanitize(editor, innerHtml))
        )
    ...
    var placeHolderConverter = function(editor) {
      return function(nodes) {
        ...
        while (i--) {
          ...
          if (node.name === 'script') {
            videoScript = $_9fg1sigejfuw8poq.getVideoScriptMatch(
              $_bmkzfegajfuw8pol.getScripts(editor),
              node.attr('src')
            )
            if (!videoScript) {
          ...
          if (
            node.name === 'iframe' &&
            $_bmkzfegajfuw8pol.hasLiveEmbeds(editor) &&
            global$1.ceFalse
          ) {
    ...
    var setup = function(editor) {
      editor.on('preInit', function() {
        ...
        editor.parser.addNodeFilter(
          ...
          $_bp9pftgsjfuw8ppl.placeHolderConverter(editor)
        )
        editor.serializer.addAttributeFilter('data-mce-object', function(
          ...
          while (i--) {
            ...
            if (innerHtml) {
              ...
              innerNode.raw = true
              innerNode.value = $_77d5ybgtjfuw8ppp.sanitize(
                editor,
                unescape(innerHtml)
              )
        ...
        if (target.getAttribute('data-mce-object')) {
          ...
          if (html) {
            ...
            target.setAttribute(
              'data-mce-html',
              escape(
                $_9oq1mlgkjfuw8pp0.updateHtml(html, {
                  width: e.width,
                  height: e.height
                })
              )
