;(function() {
  var inlite = (function() {
    ...
    var getSelectionRect = function(editor) {
      ...
      return clientRect
        ? toAbsolute($_1tf7l3171jfuw8we0.fromClientRect(clientRect))
        : null
    }
      ...
      return function(editor) {
        for (var i = 0; i < predicateIds.length; i++) {
          if (predicateIds[i].predicate(element)) {
            var result = $_8rky1516zjfuw8wdx.result(
              predicateIds[i].id,
              $_eiro29170jfuw8wdy.getElementRect(editor, element)
            )
            return result
          }
        }
        ...
        for (var i = 0; i < elements.length; i++) {
          for (var x = 0; x < predicateIds.length; x++) {
            if (predicateIds[x].predicate(elements[i])) {
              return $_8rky1516zjfuw8wdx.result(
                predicateIds[x].id,
                $_eiro29170jfuw8wdy.getElementRect(editor, elements[i])
              )
            }
          }
        }
    ...
    var textSelection = function(id) {
      return function(editor) {
        if (!editor.selection.isCollapsed()) {
          var result = $_8rky1516zjfuw8wdx.result(
            id,
            $_eiro29170jfuw8wdy.getSelectionRect(editor)
          )
          return result
        }
        return null
    ...
    var getPositionHandler = function(editor) {
      return $_r1zl2179jfuw8we9.getHandlerOr(
        editor,
        'inline_toolbar_position_handler',
        $_4f0zwc17bjfuw8wee.defaultHandler
      )
    }
    ...
    var findMatchResult = function(editor, toolbars) {
      ...
      contextToolbarsPredicateIds = $_g3lgux172jfuw8we1.fromContextToolbars(
        toolbars
      )
    ...
    var overrideLinkShortcut = function(editor, panel) {
      ...
      editor.shortcuts.add('meta+k', '', function() {
        var toolbars = getToolbars(editor)
        var result = $_8rky1516zjfuw8wdx.match(editor, [
          $_9xiw90174jfuw8we2.textSelection('quicklink')
        ])
        if (result) {
          panel.show(editor, result.id, result.rect, toolbars)
        }
    ...
    var from$1 = $_d02u5u17hjfuw8wey.isFunction(Array.from)
      ? Array.from
      : function(x) {
          return slice.call(x)
        }
    ...
    var proto$1 = {
      ...
      initLayoutRect: function() {
        ...
        borderBox = self.borderBox =
          self.borderBox || $_b17qji17ojfuw8wg0.measureBox(elm, 'border')
        self.paddingBox =
          self.paddingBox || $_b17qji17ojfuw8wg0.measureBox(elm, 'padding')
        self.marginBox =
          self.marginBox || $_b17qji17ojfuw8wg0.measureBox(elm, 'margin')
    ...
    function NotificationManagerImpl(editor) {
      ...
      var positionNotifications = function(notifications) {
        if (notifications.length > 0) {
          ...
          $_1doq1817ejfuw8wen.each(notifications, function(
            notification,
            index
          ) {
            if (index > 0) {
    ...
    var FloatPanel = Panel.extend({
      Mixins: [$_cn53y617xjfuw8wgp, $_f8jegf18ajfuw8wit],
      init: function(settings) {
        var self = this
        self._super(settings)
    ...
    var getSelectedAnchors = function(editor) {
      ...
      startElm = getParentAnchorOrSelf(
        dom,
        global$15.getNode(rng.startContainer, rng.startOffset)
      )
    ...
    var convertLinkToAbsolute = function(editor, href) {
      return !$_7ouuxp18tjfuw8wkn.isAbsolute(href) &&
        $_7ouuxp18tjfuw8wkn.isDomainLike(href)
        ? askAboutPrefix(editor, href)
        : global$13.resolve(href)
    }
    ...
    var create$3 = function() {
      ...
      var create = function(editor, toolbars) {
        var items = createToolbars(editor, toolbars).concat([
          $_8elil818ujfuw8wko.create(
            editor,
            'text',
            $_44ho17177jfuw8we6.getTextSelectionToolbarItems(editor)
          ),
          $_8elil818ujfuw8wko.create(
            editor,
            'insert',
            $_44ho17177jfuw8we6.getInsertToolbarItems(editor)
          ),
      ...
      var repositionPanelAt = function(panel, id, editor, targetRect) {
        ...
        if (id === 'insert') {
          result = $_4f0zwc17bjfuw8wee.calcInsert(
            targetRect,
            contentAreaRect,
            panelRect
          )
        } else {
          result = $_4f0zwc17bjfuw8wee.calc(
            targetRect,
            contentAreaRect,
            panelRect
          )
        ...
        if (result) {
          ...
          movePanelTo(
            panel,
            $_4f0zwc17bjfuw8wee.userConstrain(
              userConstainHandler,
              targetRect,
              contentAreaRect,
              transposedPanelRect
    ...
    var isComment = function(element) {
      return (
        type(element) === $_8v84g119ojfuw8wnd.COMMENT ||
    ...
    var inBody = function(element) {
      var dom = $_a06ojb19njfuw8wnc.isText(element)
    ...
    function MixedBag(required, optional) {
      ...
      return function(obj) {
        ...
        if (!allReqd) $_2f7evl19ujfuw8wo9.reqMessage(required, keys)
        ...
        $_1doq1817ejfuw8wen.each(optional, function(opt) {
          r[opt] = $_d9fl1s17gjfuw8wew.constant(
    ...
    var detectBrowser = function(browsers, userAgent) {
      return detect$1(browsers, userAgent).map(function(browser) {
        var version = $_g5cbf01a1jfuw8wou.detect(
    ...
    var removeLeading = function(str, prefix) {
      return startsWith(str, prefix)
        ? $_52l0y51a7jfuw8wpe.removeFromStart(str, prefix.length)
        : str
    }
    var removeTrailing = function(str, prefix) {
      return endsWith(str, prefix)
        ? $_52l0y51a7jfuw8wpe.removeFromEnd(str, prefix.length)
        : str
    }
    var ensureLeading = function(str, prefix) {
      return startsWith(str, prefix)
        ? str
        : $_52l0y51a7jfuw8wpe.addToStart(str, prefix)
    }
    var ensureTrailing = function(str, prefix) {
      return endsWith(str, prefix)
        ? str
        : $_52l0y51a7jfuw8wpe.addToEnd(str, prefix)
    }
    var contains$1 = function(str, substr) {
      return str.indexOf(substr) !== -1
    }
    var capitalize = function(str) {
      return $_7llj9o1a8jfuw8wpf
        .head(str)
        .bind(function(head) {
    ...
    var browsers = [
      {
        ...
        search: function(uastring) {
          var monstrosity =
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'edge/') &&
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'chrome') &&
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'safari') &&
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'applewebkit')
          return monstrosity
        }
      },
      {
        ...
        search: function(uastring) {
          return (
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'chrome') &&
            !$_1vh2q91a6jfuw8wpc.contains(uastring, 'chromeframe')
          )
        }
      },
      {
        ...
        search: function(uastring) {
          return (
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'msie') ||
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'trident')
          )
        }
      },
      {
      ...
      },
      {
        ...
        search: function(uastring) {
          return (
            ($_1vh2q91a6jfuw8wpc.contains(uastring, 'safari') ||
              $_1vh2q91a6jfuw8wpc.contains(uastring, 'mobile/')) &&
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'applewebkit')
          )
        }
      }
    ...
    var oses = [
      ...
      },
      {
        ...
        search: function(uastring) {
          return (
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'iphone') ||
            $_1vh2q91a6jfuw8wpc.contains(uastring, 'ipad')
          )
        },
        versionRegexes: [
          /.*?version\/\ ?([0-9]+)\.([0-9]+).*/,
    ...
    var detect$2 = function(userAgent) {
      ...
      var browser = $_kiexy1a4jfuw8wp2
        .detectBrowser(browsers, userAgent)
        .fold($_ddjyrg1a0jfuw8wor.unknown, $_ddjyrg1a0jfuw8wor.nu)
      var os = $_kiexy1a4jfuw8wp2
        .detectOs(oses, userAgent)
        .fold($_2otgg1a2jfuw8wow.unknown, $_2otgg1a2jfuw8wow.nu)
    ...
    var all = function(selector, scope) {
      ...
      return bypassSelector(base)
        ? []
        : $_1doq1817ejfuw8wen.map(
            base.querySelectorAll(selector),
            $_fmubbj19ijfuw8wmy.fromDom
          )
    }
    ...
    var member = function(element, elements) {
      return $_1doq1817ejfuw8wen.exists(
        elements,
        $_d9fl1s17gjfuw8wew.curry(eq, element)
      )
    }
    ...
    var parents = function(element, isRoot) {
      var stop = $_d02u5u17hjfuw8wey.isFunction(isRoot)
        ? isRoot
        : $_d9fl1s17gjfuw8wew.constant(false)
      var dom = element.dom()
      var ret = []
    ...
    var prevSiblings = function(element) {
      return $_1doq1817ejfuw8wen.reverse(
        $_alx4av19vjfuw8woa.toArray(element, prevSibling)
      )
    }
    ...
    var ancestors = function(scope, predicate, isRoot) {
      return $_1doq1817ejfuw8wen.filter(
        $_g7tj5v19pjfuw8wnf.parents(scope, isRoot),
        predicate
      )
    }
    var siblings$1 = function(scope, predicate) {
      return $_1doq1817ejfuw8wen.filter(
        $_g7tj5v19pjfuw8wnf.siblings(scope),
        predicate
      )
    }
    var children$1 = function(scope, predicate) {
      return $_1doq1817ejfuw8wen.filter(
        $_g7tj5v19pjfuw8wnf.children(scope),
        predicate
      )
    }
    var descendants = function(scope, predicate) {
      var result = []
      $_1doq1817ejfuw8wen.each($_g7tj5v19pjfuw8wnf.children(scope), function(
        x
      ) {
        if (predicate(x)) {
    ...
    var ancestors$1 = function(scope, selector, isRoot) {
      return $_bavy9s19kjfuw8wn5.ancestors(
        scope,
        function(e) {
    ...
    var select = function(selector, root) {
      return $_1doq1817ejfuw8wen.map(
        $_b2no8419jjfuw8wn4.descendants(
          $_fmubbj19ijfuw8wmy.fromDom(root),
          selector
        ),
        function(element) {
          return element.dom()
    ...
    var anchorTarget = function(elm) {
      ...
      return create$4(
        ...
        '#' + anchorId,
        0,
        $_d9fl1s17gjfuw8wew.noop
      )
    }
    var getHeaderTargets = function(elms) {
      return $_1doq1817ejfuw8wen.map(
        $_1doq1817ejfuw8wen.filter(elms, isValidHeader),
        headerTarget
      )
    }
    var getAnchorTargets = function(elms) {
      return $_1doq1817ejfuw8wen.map(
        $_1doq1817ejfuw8wen.filter(elms, isValidAnchor),
        anchorTarget
      )
    }
    ...
    var createMenuItems = function(term, targets, fileType, editorSettings) {
      ...
      var fromHistoryMenuItems = function(history) {
        var historyItems = history.hasOwnProperty(fileType)
          ? history[fileType]
          : []
        var uniqueHistory = $_1doq1817ejfuw8wen.filter(historyItems, function(
          url
        ) {
      ...
      var fromMenuItems = function(type) {
        var filteredTargets = $_1doq1817ejfuw8wen.filter(targets, function(
          target
        ) {
      ...
      var join = function(items) {
        return $_1doq1817ejfuw8wen.foldl(
          items,
          function(a, b) {
    ...
    function ClosestOrAncestor(is, ancestor, scope, a, isRoot) {
      return is(scope, a)
        ? Option.some(scope)
        : $_d02u5u17hjfuw8wey.isFunction(isRoot) && isRoot(scope)
          ? Option.none()
    ...
    var sibling = function(scope, predicate) {
      ...
      return child$1($_fmubbj19ijfuw8wmy.fromDom(element.parentNode), function(
        x
      ) {
    ...
    var child$1 = function(scope, predicate) {
      var result = $_1doq1817ejfuw8wen.find(
        scope.dom().childNodes,
        $_d9fl1s17gjfuw8wew.compose(
          predicate,
          $_fmubbj19ijfuw8wmy.fromDom
        )
      )
      ...
      var descend = function(element) {
        for (var i = 0; i < element.childNodes.length; i++) {
          if (predicate($_fmubbj19ijfuw8wmy.fromDom(element.childNodes[i])))
            return Option.some(
              $_fmubbj19ijfuw8wmy.fromDom(element.childNodes[i])
            )
    ...
    var ancestor$1 = function(scope, selector, isRoot) {
      return $_7vqge21afjfuw8wq8.ancestor(
        scope,
        function(e) {
          return $_9hz7rv1a9jfuw8wpg.is(e, selector)
        },
      ...
      global$4.each(items, function(item) {
        if (
          ...
          getFirstFont(item.value).toLowerCase() ===
            getFirstFont(fontFamily).toLowerCase()
        ) {
          value = item.value
        }
      })
    ...
    var createCustomMenuItems = function(editor, names) {
      ...
      if (typeof names === 'string') {
        ...
        return $_1doq1817ejfuw8wen.flatten(
    ...
    var trimMenuItems = function(menuItems) {
      var menuItems2 = $_1doq1817ejfuw8wen.filter(menuItems, function(
        menuItem,
        i,
        menuItems
      ) {
      ...
      return $_1doq1817ejfuw8wen.filter(menuItems2, function(
        menuItem,
        i,
        menuItems
      ) {
    ...
    var setupUiContainer = function(editor) {
      if (editor.settings.ui_container) {
        global$1.container = $_fms31h1aejfuw8wq6
          .descendant(
            $_fmubbj19ijfuw8wmy.fromDom(document.body),
            editor.settings.ui_container
          )
          .fold($_d9fl1s17gjfuw8wew.constant(null), function(elm) {
            return elm.dom()
          })
      }
    }
    ...
    var Slider = Widget.extend({
      init: function(settings) {
        ...
        self._minValue = $_d02u5u17hjfuw8wey.isNumber(settings.minValue)
          ? settings.minValue
          : 0
        self._maxValue = $_d02u5u17hjfuw8wey.isNumber(settings.maxValue)
          ? settings.maxValue
          : 100
