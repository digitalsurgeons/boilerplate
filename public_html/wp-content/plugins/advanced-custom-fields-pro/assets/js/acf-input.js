...
*/
  ...
  acf.fields = acf.model.extend({
    ...
    _sortstart_field: function($el, $placeholder) {
      acf.do_action(
        'sortstart_field/type=' + $el.data('type'),
        $el,
        $placeholder
      )
    },
    ...
    _sortstop_field: function($el, $placeholder) {
      acf.do_action(
        'sortstop_field/type=' + $el.data('type'),
        $el,
        $placeholder
      )
    },
  ...
  acf.layout = acf.model.extend({
    ...
    render_table: function($table) {
      ...
      $table
        .find('.acf-row.-collapsed .acf-field.-collapsed-target')
  ...
  acf.tooltip = acf.model.extend({
    ...
    confirm: function($el, callback, text, button_y, button_n) {
      ...
      button_y =
        button_y ||
        '<a href="#" class="acf-confirm-y">' + acf._e('yes') + '</a>'
      button_n =
        button_n || '<a href="#" class="acf-confirm-n">' + acf._e('No') + '</a>'
    ...
    confirm_remove: function($el, callback) {
      ...
      text = false // default
      button_y =
        '<a href="#" class="acf-confirm-y -red">' + acf._e('remove') + '</a>'
      button_n =
        '<a href="#" class="acf-confirm-n">' + acf._e('cancel') + '</a>'
...
;(function($) {
  acf.conditional_logic = acf.model.extend({
    ...
    calculate_checkbox: function(rule, $trigger) {
      ...
      var match = $trigger
        .find('input[value="' + rule.value + '"]:checked')
        .exists()
...
*/
      ...
      frame.on(
        ...
        function() {
          ...
          this.$el
            .closest('.media-modal')
            .addClass('acf-media-modal -' + settings.mode)
        },
        frame
      )
      ...
      frame.on('content:activate:browse', function() {
        ...
        if (settings.library == 'uploadedTo') {
          ...
          filters.$el
            .parent()
            .append(
              '<span class="acf-uploadedTo">' +
                acf._e('image', 'uploadedTo') +
                '</span>'
            )
    ...
    customize_Attachment: function() {
      ...
      wp.media.view.Attachment.Library = AttachmentLibrary.extend({
        ...
        toggleSelection: function(options) {
          ...
          var frame = acf.media.frame(),
            errors = acf.maybe_get(this, 'model.attributes.acf_errors'),
            $sidebar = this.controller.$el.find(
    ...
    customize_AttachmentCompat: function() {
      ...
      wp.media.view.AttachmentCompat = AttachmentCompat.extend({
        add_acf_expand_button: function() {
          ...
          if ($el.find('.media-frame-router .acf-expand-details').exists())
            return
    ...
    render: function(json) {
      ...
      if (!json || !json.results || !json.results.length) {
        ...
        if (this.o.paged == 1) {
          this.$choices
            .children('.list')
            .append('<p>' + acf._e('relationship', 'empty') + '</p>')
        }
      ...
      this.$values.find('.acf-rel-item').each(function() {
        $new
          .find('.acf-rel-item[data-id="' + $(this).data('id') + '"]')
          .addClass('disabled')
      })
...
;(function($) {
  ...
  _select23 = _select2.version3 = {
    ready: function() {
      ...
      $.fn.select2.locales = acf.maybe_get(
        window,
        'jQuery.fn.select2.locales',
        {}
      )
    ...
    init: function($select, args, $field) {
      ...
      var select2_args = {
        ...
        formatResult: function(result, container, query, escapeMarkup) {
          ...
          if (result.description) {
            text +=
              ' <span class="select2-result-description">' +
              result.description +
              '</span>'
          }
      ...
      // filter for 3rd party customization
      select2_args = acf.apply_filters(
        'select2_args',
        select2_args,
        $select,
        args,
...
;(function($) {
  acf.fields.tab = acf.field.extend({
    ...
    hide: function($field, context) {
      ...
      setTimeout(function() {
        ...
        if ($li.hasClass('active')) {
          $group
            .find('li:not(.hidden-by-conditional-logic):first a')
            .trigger('click')
        }
      }, 0)
    },
    ...
    show: function($field, context) {
      ...
      setTimeout(function() {
        ...
        if (
          !$active.exists() ||
          $active.hasClass('hidden-by-conditional-logic')
        ) {
          $a.trigger('click')
        }
      }, 0)
  ...
  var tab_manager = acf.model.extend({
    ...
    render: function($el) {
      ...
      $('.acf-tab-wrap', $el).each(function() {
        ...
        if ($wrap.hasClass('-sidebar')) {
          ...
          var height =
            $group.position().top + $group.children('ul').outerHeight(true) - 1
    ...
    add_tab: function($field, settings) {
      //console.log('add_tab(%o, %o)', $field, settings);
      ...
      // vars
      var $li = $(
        '<li><a class="acf-tab-button" href="#" data-key="' +
          settings.key +
          '">' +
          settings.text +
          '</a></li>'
...
;(function($) {
  ...
  acf.fields.taxonomy = acf.field.extend({
    ...
    append_new_term: function(term) {
      ...
      $('.acf-taxonomy-field[data-taxonomy="' + this.o.taxonomy + '"]').each(
        function() {
          // vars
...
*/
        ...
        case 'radio':
          ...
          var $holder = this.$el.find('.categorychecklist-holder'),
            ...
            offet =
              $holder.get(0).scrollTop +
              ($li.offset().top - $holder.offset().top)
...
;(function($) {
  acf.validation = acf.model.extend({
    ...
    ready: function($el) {
      // reference
      $el
        .find('.acf-field input')
        .filter('[type="number"], [type="email"], [type="url"]')
    ...
    fetch_success: function($form, json) {
      ...
      if (!$message.exists()) {
        $message = $(
          '<div class="acf-error-message"><p></p><a href="#" class="acf-icon -cancel small"></a></div>'
        )
