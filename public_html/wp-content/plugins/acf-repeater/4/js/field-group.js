;(function($) {
  var repeater = {
    ...
    render: function() {
      ...
      // find layout value
      if (
        this.$el.find('input[name="fields[' + id + '][layout]"]:checked')
          .length > 0
      ) {
        layout = this.$el
          .find('input[name="fields[' + id + '][layout]"]:checked')
          .val()
      }
      ...
      this.$el
        .find('.repeater:first')
        .removeClass('layout-row layout-table')
  ...
  $(document).on(
    'click',
    '.field_option_repeater_layout input[type="radio"]',
    function(e) {
