;(function($) {
  ...
  acf.fields.flexible_content = {
    ...
    render: function() {
      ...
      this.$values.children('.layout').each(function(i) {
        $(this)
          .find('> .acf-fc-layout-handle .fc-layout-order')
          .html(i + 1)
      })
      ...
      if (this.o.layout_count >= this.o.max) {
        ...
        this.$el
          .find('> .acf-flexible-content-footer .acf-button')
          .addClass('disabled')
      } else {
    ...
    validate_remove: function(layout) {
      ...
      if (this.o.min > 0 && this.o.layout_count <= this.o.min) {
        var identifier = this.o.min == 1 ? 'layout' : 'layouts',
          s =
            acf.l10n.flexible_content.min +
            ', ' +
            acf.l10n.flexible_content.remove
      ...
      if (layout_min > 0 && layout_count <= layout_min) {
        var identifier = layout_min == 1 ? 'layout' : 'layouts',
          s =
            acf.l10n.flexible_content.min_layout +
            ', ' +
            acf.l10n.flexible_content.remove
    ...
    add: function(layout, $before) {
      ...
      var new_id = acf.helpers.uniqid(),
        new_field_html = this.$el
          .find('> .clones > .layout[data-layout="' + layout + '"]')
          .html()
          .replace(/(=["]*[\w-\[\]]*?)(acfcloneindex)/g, '$1' + new_id),
        new_field = $(
          '<div class="layout" data-layout="' + layout + '"></div>'
    ...
    open_popup: function($a, in_layout) {
      ...
      $popup.find('a').each(function() {
        ...
        var min = parseInt($(this).attr('data-min')),
          ...
          label = $(this).text(),
          count = _this.$values.children('.layout[data-layout="' + name + '"]')
            .length,
  ...
  $(document).on(
    'click',
    '.acf-flexible-content .flexible-footer .acf-fc-add',
    function(e) {
      e.preventDefault()

      acf.fields.flexible_content
        .set({ $el: $(this).closest('.acf-flexible-content') })
        .open_popup($(this))
  ...
  $(document).on(
    'click',
    '.acf-flexible-content .acf-fc-layout-controlls .acf-fc-add',
    function(e) {
      e.preventDefault()

      acf.fields.flexible_content
        .set({ $el: $(this).closest('.acf-flexible-content') })
        .open_popup($(this), true)
  ...
  $(document).on(
    'click',
    '.acf-flexible-content .acf-fc-layout-controlls .acf-fc-remove',
    function(e) {
      e.preventDefault()

      acf.fields.flexible_content
        .set({ $el: $(this).closest('.acf-flexible-content') })
        .remove($(this).closest('.layout'))
  ...
  $(document).on('click', '.acf-flexible-content .acf-fc-popup li a', function(
    e
  ) {
    e.preventDefault()
    ...
    acf.fields.flexible_content
      .set({ $el: $(this).closest('.acf-flexible-content') })
      .add($(this).attr('data-layout'), $layout)

    $(this).blur()
  })

  $(document).on('blur', '.acf-flexible-content .acf-fc-popup .focus', function(
    e
  ) {
  ...
  $(document).on('acf/validate_field', function(e, field) {
    ...
    $popup.find('a').each(function() {
      ...
      var min = parseInt($(this).attr('data-min')),
        ...
        count = $el
          .children('.values')
          .children('.layout[data-layout="' + name + '"]').length
